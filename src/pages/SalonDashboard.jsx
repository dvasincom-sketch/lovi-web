...
function StrategyDrawer({ open, onClose, services, token, onSaved }) {
  const [step, setStep]         = useState(0)  // 0=priority, 1=discount, 2=horizon, 3=services
      ...
      <div style={drawerStyle}>
        ...
        <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:24}}>
          ...
          <button onMouseDown={onClose}
            style={{background:"none", border:"none", fontSize:20, color:"#8F8475",
        ...
        <div style={{display:"flex", gap:4, marginBottom:28}}>
          {Array.from({length: totalSteps}).map((_, i) => (
            <div key={i} style={{flex:1, height:3, borderRadius:2,
              background: i <= step ? "#121A12" : "rgba(18,26,18,0.1)",
        ...
        {step === 0 && (
          <div>
            ...
            {PRIORITY_OPTIONS.map(opt => (
              <button key={opt.id} style={optionStyle(priority === opt.id)}
        ...
        {step === 1 && (
          <div>
            <p style={{fontSize:13, color:"#8F8475", margin:"0 0 20px", lineHeight:1.6}}>
              Максимальная скидка которую увидит клиент когда до слота остался час.
              Чем больше скидка — тем выше вероятность что горящий слот забронируют.
            </p>
            {DISCOUNT_PRESETS.map(p => (
              <button key={p.id} style={optionStyle(discount === p.id)}
        ...
        {step === 2 && (
          <div>
            <p style={{fontSize:13, color:"#8F8475", margin:"0 0 20px", lineHeight:1.6}}>
              Главная сила Lovi — горящие окошки в ближайшие часы.
              Скидки заранее — дополнительный инструмент для заполнения расписания на неделю.
            </p>
            {HORIZON_PRESETS.map(p => (
              <button key={p.id} style={optionStyle(horizon === p.id)}
          ...
          <div>
            ...
            <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:12}}>
              ...
              <button onClick={() => {
                ...
                setChecked(next)
              }} style={{background:"none", border:"none", fontSize:12, color:"#F97316",
